...
class Tap
  ...
  def self.fetch(user, repository = T.unsafe(nil))
    ...
    user = user.capitalize if ["homebrew", "linuxbrew", "cerisola"].include?(user)
    repository = repository.sub(HOMEBREW_OFFICIAL_REPO_PREFIXES_REGEX, "")

    return CoreTap.instance if ["Homebrew", "Linuxbrew", "cerisola"].include?(user) && ["core", "homebrew"].include?(repository)
    return CoreCaskTap.instance if user == "cerisola" && repository == "cask"
