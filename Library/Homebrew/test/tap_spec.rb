...
RSpec.describe Tap do
  ...
  specify "#issues_url" do
    ...
    expect(homebrew_foo_tap.issues_url).to eq("https://github.com/cerisola/homebrew-foo/issues")
  ...
  describe "#remote" do
    it "returns the remote URL", :needs_network do
      ...
      expect(homebrew_foo_tap.remote).to eq("https://github.com/cerisola/homebrew-foo")
      ...
      services_tap.path.cd do
        system "git", "init"
        system "git", "remote", "add", "origin", "https://github.com/cerisola/homebrew-test-bot"
      end
  ...
  describe "#custom_remote?" do
    ...
    context "when using the default remote" do
      let(:remote) { "https://github.com/cerisola/homebrew-test-bot" }

      it(:custom_remote?) { expect(tap.custom_remote?).to be false }
    end

    context "when using a non-default remote" do
      let(:remote) { "git@github.com:cerisola/homebrew-test-bot" }
